...
- **Нарушение принципа единственной ответственности** — класс `Teacher` вынужден знать о логике поражения в игре (`renpy.jump`), интерфейсе уведомлений и состоянии объекта `Student`.
- **Нарушение принципа открытости/закрытости** — если мы захотим добавить новое последствие гнева (например, разблокировку достижения или реакцию других студентов в аудитории), нам придется менять класс `Teacher`.
- **Сильная связанность** — преподаватель жестко связан с конкретными экземплярами и глобальным состоянием визуальной новеллы.
...
| `StressRelay` | Выступает в роли связующего звуна. При инициализации получает ссылку на `Student`. При обновлении считывает гнев учителя и динамически пересчитывает уровень стресса студента с элементом псевдослучайности. |
...
Внедрение паттерна Наблюдатель повлияло на проект следующим образом:

**Слабая связанность.** Класс `Teacher` теперь ничего не знает об объекте `Student`, логике проигрыша (`DeathObserver`) или механике стресса. Он отвечает только за свои собственные данные (имя, гнев).

**Расширяемость.** Если в игру потребуется добавить логику, при которой из-за криков преподавателя в аудиторию заходит декан, достаточно будет создать новый класс наследуемый от `Observer` и сделать `radmir.attach(New_Observer())`. Исходный код классов `Teacher` или `Student` вообще не придется менять.
...
https://www.transfernow.net/dl/20260422rQFwwJGf
